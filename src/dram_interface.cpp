...
void dram_interface::cycle(){
    ...
    if(m_cur_dram_cycle >= mf->access_complete_cycle){ // Memory request has compeleted
        
        // Increment stats
        if(mf->type == READ) m_num_reads++;
        else m_num_writes++;
...
bool dram_interface::do_access(memory_fetch *mf){
    ...
    if(m_mem_queue.size() < m_max_pending_req){
        ...
        mf->access_complete_cycle = (m_cur_dram_cycle + m_access_latency);
